...
(async()=>{let browser;try{
 ...
 // Keep the history integration check independent of the separate live line feed.
 await page.route('**/api/lines',r=>r.fulfill({contentType:'application/json',body:JSON.stringify({lines:[],retrieved_at:new Date().toISOString(),expires_at:new Date(Date.now()+90000).toISOString(),complete:true})}));
 ...
 assert.equal(await page.locator('#projection').isDisabled(),true,'Missing live lines must not become manual or invented lines');
